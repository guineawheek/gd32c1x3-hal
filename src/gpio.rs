    ...
    PIN::Mode: Interruptable,
{
    ...
    fn make_interrupt_source(&mut self, afio: &mut afio::Parts) {
        ...
        match pin_number {
            0..=3 => {
                afio.extiss0.extiss0().modify(|r, w| unsafe {
                    w.bits((r.bits() & !(0xf << offset)) | (port << offset))
                });
            }
            4..=7 => {
                afio.extiss1.extiss1().modify(|r, w| unsafe {
                    w.bits((r.bits() & !(0xf << offset)) | (port << offset))
                });
            }
            8..=11 => {
                afio.extiss2.extiss2().modify(|r, w| unsafe {
                    w.bits((r.bits() & !(0xf << offset)) | (port << offset))
                });
            }
            12..=15 => {
                afio.extiss3.extiss3().modify(|r, w| unsafe {
                    w.bits((r.bits() & !(0xf << offset)) | (port << offset))
                });
            }
            _ => unreachable!(),
    ...
    fn clear_interrupt_pending_bit(&mut self) {
        unsafe { (*Exti::ptr()).pd().write(|w| w.bits(1 << self.pin_id())) };
    }

    /// Reads the interrupt pending bit for this pin
    fn check_interrupt(&self) -> bool {
        unsafe { ((*Exti::ptr()).pd().read().bits() & (1 << self.pin_id())) != 0 }
    }
}
    ...
    Self: HL,
{
    fn mode<MODE: PinMode>(&mut self, cr: &mut <Self as HL>::Cr) {
        ...
        if let Some(pull) = MODE::PULL {
            if pull {
                gpio.bop().write(|w| unsafe { w.bits(1 << N) });
            } else {
                gpio.bc().write(|w| unsafe { w.bits(1 << N) });
            }
        }
...
impl<const P: char> Gpio<P> {
    const fn ptr() -> *const pac::gpioa::RegisterBlock {
        match P {
            'A' => pac::Gpioa::ptr(),
