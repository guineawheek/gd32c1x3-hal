...
use crate::bb;
use crate::pac::{Dbg, Rcu};
use crate::pac;

use crate::rcu::{self, Clocks};
...
pub trait Instance:
    crate::Sealed + rcu::Enable + rcu::Reset + rcu::BusTimerClock + General
{
}
    ...
    ($($TIM:ty: [
        ...
        $(
            ...
            impl General for $TIM {
                ...
                #[inline(always)]
                fn enable_preload(&mut self, b: bool) {
                    self.ctl0().modify(|_, w| w.arse().bit(b));
                }
                #[inline(always)]
                fn enable_counter(&mut self) {
                    self.ctl0().modify(|_, w| w.cen().set_bit());
                }
                #[inline(always)]
                fn disable_counter(&mut self) {
                    self.ctl0().modify(|_, w| w.cen().clear_bit());
                }
                #[inline(always)]
                fn is_counter_enabled(&self) -> bool {
                    self.ctl0().read().cen().bit_is_set()
                }
                #[inline(always)]
                fn reset_counter(&mut self) {
                    self.cnt().reset();
                ...
                fn trigger_update(&mut self) {
                    ...
                    // the UG bit
                    self.ctl0().modify(|_, w| w.ups().set_bit());
                    self.swevg().write(|w| w.upg().set_bit());
                    self.ctl0().modify(|_, w| w.ups().clear_bit());
                }
                #[inline(always)]
                fn clear_interrupt_flag(&mut self, event: Event) {
                    self.intf().write(|w| unsafe { w.bits(0xffff & !event.bits()) });
                }
                #[inline(always)]
                fn listen_interrupt(&mut self, event: Event, b: bool) {
                    if b {
                        self.dmainten().modify(|r, w| unsafe { w.bits(r.bits() | event.bits()) });
                    } else {
                        self.dmainten().modify(|r, w| unsafe { w.bits(r.bits() & !event.bits()) });
                    }
                }
                #[inline(always)]
                fn get_interrupt_flag(&self) -> Event {
                    Event::from_bits_truncate(self.intf().read().bits())
                }
                #[inline(always)]
                fn read_count(&self) -> Self::Width {
                ...
                #[inline(always)]
                fn start_one_pulse(&mut self) {
                    self.ctl0().write(|w| unsafe { w.bits(1 << 3) }.cen().set_bit());
                }
                #[inline(always)]
                fn ctl0_reset(&mut self) {
                    self.ctl0().reset();
                ...
                #[inline(always)]
                fn stop_in_debug(&mut self, dbg: &mut Dbg, state: bool) {
                    dbg.ctl0().modify(|_, w| w.$dbg_timX_stop().bit(state));
                }
            }
            $(with_pwm!($TIM: $cnum $(, $aoe)?);)?

            $(impl MasterTimer for $TIM {
                type Mms = crate::pac::$timbase::ctl1::Mmc;
                fn master_mode(&mut self, mode: Self::Mms) {
                    self.ctl1().modify(|_,w| w.mmc().variant(mode));
                }
            })?
        )+
    }
...
macro_rules! with_pwm {
    ($TIM:ty: CH1) => {
        impl WithPwm for $TIM {
            ...
            #[inline(always)]
            fn start_pwm(&mut self) {
                self.ctl0.write(|w| w.cen().set_bit());
            }
    ...
    ($TIM:ty: CH4 $(, $aoe:ident)?) => {
        impl WithPwm for $TIM {
            ...
            fn read_cc_value(channel: u8) -> u32 {
                let tim = unsafe { &*<$TIM>::ptr() };
                match channel {
                    0 => tim.ch0cv().read().bits(),
                    1 => tim.ch1cv().read().bits(),
                    2 => tim.ch2cv().read().bits(),
                    3 => tim.ch3cv().read().bits(),
                    _ => 0,
                }
            }
            ...
            fn set_cc_value(channel: u8, value: u32) {
                let tim = unsafe { &*<$TIM>::ptr() };
                match channel {
                    0 => tim.ch0cv().write(|w| unsafe { w.bits(value) }),
                    1 => tim.ch1cv().write(|w| unsafe { w.bits(value) }),
                    2 => tim.ch2cv().write(|w| unsafe { w.bits(value) }),
                    3 => tim.ch3cv().write(|w| unsafe { w.bits(value) }),
                    _ => {}
                }
            }
            ...
            fn preload_output_channel_in_mode(&mut self, channel: Channel, mode: Ocm) {
                match channel {
                    Channel::C1 => {
                        self.chctl0_output()
                        .modify(|_, w| w.ch0comsen().set_bit().ch0comctl().bits(mode as _));
                    }
                    Channel::C2 => {
                        self.chctl0_output()
                        .modify(|_, w| w.ch1comsen().set_bit().ch1comctl().bits(mode as _));
                    }
                    Channel::C3 => {
                        self.chctl1_output()
                        .modify(|_, w| w.ch2comsen().set_bit().ch2comctl().bits(mode as _));
                    }
                    Channel::C4 => {
                        self.chctl1_output()
                        .modify(|_, w| w.ch3comsen().set_bit().ch3comctl().bits(mode as _));
                    }
                }
            }
            ...
            #[inline(always)]
            fn start_pwm(&mut self) {
                $(let $aoe = self.cchp().modify(|_, w| w.oaen().set_bit());)?
                self.ctl0().write(|w| w.cen().set_bit());
            }
            ...
            fn enable_channel(c: u8, b: bool) {
                let tim = unsafe { &*<$TIM>::ptr() };
                if c < Self::CH_NUMBER {
                    unsafe { bb::write(&tim.chctl2(), c*4, b); }
                }
            }
        }
    }
...
hal!(
    pac::Timer1: [Timer1, u16, timer1_hold, c: (CH4), m: timer1,],
    pac::Timer2: [Timer2, u16, timer2_hold, c: (CH4), m: timer1,],
    pac::Timer3: [Timer3, u16, timer3_hold, c: (CH4), m: timer1,],
    pac::Timer4: [Timer4, u16, timer4_hold, c: (CH4), m: timer1,],
);

hal!(
    pac::Timer0: [Timer0, u16, timer0_hold, c: (CH4, _aoe), m: timer0,],
    pac::Timer7: [Timer7, u16, timer7_hold, c: (CH4, _aoe), m: timer0,],

);

hal! {
    pac::Timer5: [Timer5, u16, timer5_hold, m: timer5,],
    pac::Timer6: [Timer6, u16, timer6_hold, m: timer5,],
...
*/
// hal! {
//     gd32c103::TIMER9: (tim9, timer9_halt),
//     gd32c103::TIMER10: (tim10, dbg_tim10_stop),
//     gd32c103::TIMER11: (tim11, dbg_tim11_stop),
// }
